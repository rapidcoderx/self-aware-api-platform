...
export default function Solution() {
  return (
    <section id="solution" className="relative z-10 flex flex-col justify-center pt-16 pb-2 px-6 md:px-12 lg:px-24">
      <div className="max-w-7xl mx-auto">
        <SectionHeader title="The Solution" tag="// APPROACH" />
        <p className="font-display text-xl md:text-2xl text-center italic text-gradient-gold mb-5 max-w-3xl mx-auto">
          "Turn API specs into living infrastructure — observable, validated, and self-healing."
        </p>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          {PILLARS.map((p) => (
            <div key={p.title}
              className="glass-strong rounded-2xl p-5 text-center hover:border-accent-primary/40 transition-colors duration-200">
              <div className="text-3xl mb-2">{p.icon}</div>
              <h3 className="font-display font-bold text-star-white text-base mb-2">{p.title}</h3>
